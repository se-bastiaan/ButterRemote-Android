...
public class AutoUpdateApk extends Observable {
    ...
    //
    public AutoUpdateApk(Context ctx, String appKey) {
        setupVariables(ctx, appKey);
    }
...
//
    ...
    private final static String API_URL = "http://unanap.se-bastiaan.eu/api/check";
    ...
    private void setupVariables(Context ctx, String appKeyString) {
        context = ctx;
...
//		schedule.add(new ScheduleEntry(0,24));
    ...
    private class checkUpdateTask extends AsyncTask<Void,Void,String[]> {
        ...
        protected String[] doInBackground(Void... v) {
            ...
            try {
                StringEntity params = new StringEntity("version=" + versionCode +
                        "&md5=" + preferences.getString( MD5_KEY, "0") +
                        "&id=" + String.format( "%08x", device_id) + "&app_key=" + appKey );
                post.setHeader("Content-Type", "application/x-www-form-urlencoded");
                post.setEntity(params);
                String response = EntityUtils.toString( httpclient.execute( post ).getEntity(), "UTF-8" );
                Log_v(TAG, "got a reply from update server: " + response);
